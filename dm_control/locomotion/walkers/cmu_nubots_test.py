...
from dm_control.locomotion.walkers import cmu_nubots


class CMUHumanoidTest(parameterized.TestCase):

  @parameterized.parameters([
      cmu_nubots.CMUHumanoid,
      cmu_nubots.CMUHumanoidPositionControlled,
  ])
  ...
  def test_cmu_nubots_position_controlled_has_correct_actuators(self):
    walker_torque = cmu_nubots.CMUHumanoid()
    walker_pos = cmu_nubots.CMUHumanoidPositionControlled()

    actuators_torque = walker_torque.mjcf_model.find_all('actuator')
    actuators_pos = walker_pos.mjcf_model.find_all('actuator')

    actuator_pos_params = {
        params.name: params for params in cmu_nubots._POSITION_ACTUATORS}
  ...
  def test_get_element_property(self, name):
    attribute_value = getattr(cmu_nubots.CMUHumanoid(), name)
  ...
  def test_cmu_pose_to_actuation(self):
    walker = cmu_nubots.CMUHumanoidPositionControlled()
